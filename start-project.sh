...
print_info "Running backend initialization script..."
node initialize-dev.js || { print_error "Failed to initialize backend"; exit 1; }
...
# Change to frontend directory and set up environment
cd "$PROJECT_DIR/K-S-Enterprise-" || { print_error "Failed to change to frontend directory"; exit 1; }
...
bun install || { print_error "Failed to install frontend dependencies"; exit 1; }

# Start frontend server
print_header "Starting frontend website"
...
echo -e "Frontend:  ${BLUE}$PROJECT_DIR/K-S-Enterprise-/frontend.log${NC}"
echo -e "Admin:     ${BLUE}$PROJECT_DIR/ks-admin-portal/admin.log${NC}"
echo
echo -e "${YELLOW}Press Ctrl+C to stop all services${NC}"

# Handle graceful shutdown
trap "echo -e '${YELLOW}Stopping all services...${NC}'; kill $BACKEND_PID $FRONTEND_PID $ADMIN_PORTAL_PID; echo -e '${GREEN}All services stopped${NC}'; exit" SIGINT
